...
**进入兜底模式（满足任一）：**
- 🚨 用户说"帮我规划路线图"但没有明确目标（"什么都想做"）
- 🚨 用户没有提供团队产能信息（人数、迭代周期）
- 🚨 用户把路线图当成"功能清单"（路线图要有里程碑和目标绑定）

**不触发本 Skill：**
- ⛔ 每日/每周 Sprint 任务分配（用「iteration-planning」）
...
### 必须确认
1. **季度目标**：这个季度最重要的 1-3 件事是什么？
2. **团队产能**：多少人？每个 Sprint 多长时间？
3. **依赖方**：有没有跨团队/外部依赖？

### 建议确认
4. 每个目标"做成什么样"算成功？（可量化的标准）
5. 有没有必须在上线前完成的硬性截止日期？

---
如果目标不清晰，建议先开一次目标对齐会，明确 1-3 个核心目标。
如果产能信息不足，我会用估算模型给出参考范围。
```
## Skill 协同
...
| Skill | 输出内容 | 如何配合 |
|-------|---------|---------|
| 「demand-prioritization-engine」 | 需求优先级 + 版本建议 | 据此安排里程碑 |
| 用户直接提供 | 季度 OKR / 业务目标 | 直接拆解目标 |

### 下游 Skill（输出去向）
| Skill | 接收内容 | 如何配合 |
|-------|---------|---------|
| 「iteration-planning」 | Epic / Feature 列表 | 拆解为 Sprint 任务 |
| 「prd-writer」 | 里程碑中的功能 | 据此确定 PRD 范围 |
| 「release-launch」 | 版本发布计划 | 据此制定发布节奏 |

### 推荐的 Commands
- `/plan` 产品规划流程：demand-prioritization-engine -> pm-roadmap-planner -> iteration-planning
- 单独使用：`/roadmap [季度目标]`

### Skill 地图位置
```
产品层 -> 执行交付
  demand-prioritization-engine -> pm-roadmap-planner -> [里程碑] -> iteration-planning
                                                                          -> release-launch
```
