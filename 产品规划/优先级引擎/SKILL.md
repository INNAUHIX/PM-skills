...
- **与其他 Skill 的关系**：
  - 「identify-needs」Skill 输出已验证的假设清单 → 本 Skill 据此评估需求价值
  - 本 Skill 输出的版本规划 → 「pm-roadmap-planner」Skill 据此排路线图

### 适用边界

✅ **适用**：需求池已有多个候选需求，需要理性决策顺序
❌ **不适用**：探索期的假设验证排序（用「user-prioritization」）
❌ **不适用**：单个需求的深入分析（用「prd-writer」）
...
**不触发本 Skill：**
- ⛔ 探索期的假设排序（用「user-prioritization」）
- ⛔ 纯项目管理排期（用「pm-roadmap-planner」）
...
| Skill | 输出内容 | 如何配合 |
|-------|---------|---------|
| 「identify-needs」 | 已验证的高优先级假设 + 需求池 | 直接纳入需求池评分 |
| 「interview-summary」 | 用户洞察驱动的新需求 | 补充到需求池 |

### 下游 Skill（输出去向）
| Skill | 接收内容 | 如何配合 |
|-------|---------|---------|
| 「pm-roadmap-planner」 | 优先级排序结果 + 版本排期建议 | 据此安排里程碑 |
| 「prd-writer」 | 优先级高的需求 | 优先写入 PRD |
| 「iteration-planning」 | 最终排期结果 | 据此安排 Sprint |
