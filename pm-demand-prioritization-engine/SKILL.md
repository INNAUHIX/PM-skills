---
name: pm-demand-prioritization-engine
description: |
  对需求、功能、项目进行多维度优先级排序，输出版本排期建议。当用户说帮我排优先级、用RICE打分、用ICE评分、用Kano分析时使用此 Skill。 关键词：需求优先级、RICE、ICE、Kano、排优先级、功能优先级、成本收益。
---
...
- 用户要拆解 OKR（用「roadmap-planner」）

### 兜底策略

**兜底触发**：用户没有明确的需求列表或输入来源。

在开始之前，我会先确认：

1. **有哪些需求或功能需要排序？**（列出清单）
2. **排序的目的是什么？**（资源有限、时间紧迫、战略聚焦）

如果需求列表不完整，我会先帮你补充遗漏的需求维度，再进行优先级评分。
